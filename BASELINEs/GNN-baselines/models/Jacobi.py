...
from .PolyConv import JacobiConv, PolyConvFrame
...
class JacobiGNN(nn.Module):
    ...
    Args:
        ...
        comb (nn.Module): combine bases to produce the filter function.
    '''
    def __init__(self, conv: nn.Module, comb: nn.Module):
        super().__init__()
        # self.emb = emb
        self.conv = conv
        self.comb = comb

    def forward(self, x, edge_index: Tensor, batch_idx, *args): #, edge_weight: Tensor, pos: Tensor):
        '''
        pos: mask of node whose embeddings is needed.
        '''
        ...
        nemb = self.comb(self.conv(x, edge_index, edge_attr))
        graph_embs = nemb[root_idx == 1, :]

        return graph_embs

def buildModel(conv_layer, aggr, alpha, image_idx=0, **kwargs):
    conv_fn = JacobiConv
    conv = PolyConvFrame(conv_fn,
                                  depth=conv_layer,
                                  aggr=aggr,
                                  alpha=alpha,
                                  cached=False,
                                  fixed=True)
    comb = Combination(1, conv_layer + 1, False)
    gnn = JacobiGNN(conv, comb)
    return gnn
